...
export default function TitleBar({ title = 'Portfolio — bash', path = '~' }) {
  return (
    <div className="select-none rounded-t-xl bg-emerald-50 border border-b-0 border-emerald-200 px-4 py-2 flex items-center justify-between">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-3 text-xs text-emerald-700">{title}</span>
      </div>
      <div className="text-xs text-emerald-700/80">{path}</div>
    </div>
  );
}
